...
\begin{center}
    IMT2020085 Harshadeep Donapati
\end{center}
Average acceleration due to gravity = $\bar{g}$ =  
\( \displaystyle \frac{$\sum^{n}_{i=1}g\textsubscript{i}$}{N}\) = $\frac{48.02}{5}$ = 9.604 \\
\vspace{5mm}
The standard deviation $\sigma$ is defined as \\
\sigma = \sqrt{1/N \sum^{n}_{i=1g\textsubscript{i} - $\bar{g}$}^2 = $\sqrt{0.06152}$ = 0.248 m/s\textsuperscript{2}. \\
Thus, the acceleration due to gravity is likely to be within (25.4 ±
0.2 cm) and we write \\
g = 9.604 \pm 0.248 m/s\textsuperscript{2}.
\vfill
